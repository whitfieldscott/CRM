...
import type { SmsCampaignLog } from "@/types/api";
...
export default function TextCampaignHistoryPage() {
  const [rows, setRows] = useState<SmsCampaignLog[]>([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get<SmsCampaignLog[]>("/sms/history");
      setRows(data);
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Text Campaign History</h1>
        <p className="text-muted-foreground">
          SMS campaigns sent via Twilio (same logging shape as email campaigns).
        </p>
      </div>
